...
3. **Create the `api` service**:
   - Source: this repo (root). Railway auto-detects the root `Dockerfile` (configured via `railway.json`).
...
4. **Create the `web` service** (in the same Railway project):
   - Source: this repo (root). Leave Root Directory unset / blank.
   - In the service's Settings → Build, set **Dockerfile Path** to `Dockerfile.web` (or set env var `RAILWAY_DOCKERFILE_PATH=Dockerfile.web`).
